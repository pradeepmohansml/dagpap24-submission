...
from hf_token_classification_deberta import main as hf_token_classification
...
"""
This script performs span prediction and classification with debertav3 models.
"""
...
def predictions_sanity_check():
    ...
    path_to_test_preds = str(
        project_root
        / f'{params["data"]["path_to_data"]}/{params["bert"]["output_dir"]}/test_predictions_deberta.json'
    )
...
def convert_preds_to_original_format(
    ...
    for index, row in test_preds_df.iterrows():
        ...
        if len(row["preds"]) > len(orig_test_data.loc[index, "tokens"]):
            ...
            test_preds_df.at[index, "preds"] = row["preds"] + [0]*(
...
def main():
    logger.info(f"Entering main function and starting argument parser")
    project_root: Path = get_project_root()
    logger.info(f"Got project Root as : {str(project_root)}")
    """
    try:
        ...
        logger.info(f"Initialized Parser to parse args")
        args = parser.parse_args()
    except Exception as e:
        raise logger.error(f"Error initializing argument parser")
    """
    # loading config params
    with open(str(project_root / "config" / DEBERTA_CONFIG_FILE_NAME)) as f:
        params = yaml.load(f, Loader=yaml.FullLoader)
    ...
    hf_config_file_path = str(project_root / "config" / DEBERTA_CONFIG_JSON)
