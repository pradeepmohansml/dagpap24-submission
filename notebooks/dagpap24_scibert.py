...
def main():
    ...
    with open(str(project_root / "config" / SCIBERT_CONFIG_FILE_NAME )) as f:
        params = yaml.load(f, Loader=yaml.FullLoader)

    path_to_data_folder = str(project_root / params["data"]["path_to_data"])
    logger.info(f"Path to Data: {path_to_data_folder}")
    ...
    hf_config_file_path = str(project_root / "config" / SCIBERT_CONFIG_JSON)
    ...
    path_to_test_preds = str(
        project_root
        / f'{params["data"]["path_to_data"]}/{params["bert"]["output_dir"]}/test_predictions_scibert.json'
    )
    path_to_final_output = str(
        project_root
